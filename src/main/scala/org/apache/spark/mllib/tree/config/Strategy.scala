...
package org.apache.spark.mllib.tree.config

import org.apache.spark.annotation.Since
import org.apache.spark.mllib.tree.config
...
class Strategy @Since("1.3.0") (
  ...
  /**
    * Java-friendly constructor for [[config.Strategy]]
    */
  @Since("1.1.0")
  def this(
            algo: Algo,
  ...
  def setAlgo(algo: String): Unit = algo match {
    ...
    case "LambdaMart" => setAlgo(LambdaMart)
  }
  ...
  private[tree] def assertValid(): Unit = {
    algo match {
      ...
      case LambdaMart =>
        require(impurity == Variance,
          s"DecisionTree Strategy given invalid impurity for LambdaMart: $impurity." +
            s"  Valid settings: Variance")
      case _ =>
  ...
  def defaultStrategy(algo: Algo): Strategy = algo match {
    ...
    case Algo.Regression =>
      new Strategy(algo = Regression, impurity = Variance, maxDepth = 10,
        numClasses = 0)
    case Algo.LambdaMart =>
      new Strategy(algo = LambdaMart, impurity = Variance, maxDepth = 10,
        numClasses = 0)
  }
