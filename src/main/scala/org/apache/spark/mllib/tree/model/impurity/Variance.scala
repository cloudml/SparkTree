...
private[tree] class VarianceAggregator()
   ...
   */
  def update(allStats: Array[Double], offset: Int, label: Double, instanceWeight: Double, weight: Double): Unit = {
    allStats(offset) += instanceWeight
    allStats(offset + 1) += instanceWeight * label
    allStats(offset + 2) += instanceWeight * label * label
    allStats(offset + 3) += weight
  }
...
private[tree] class VarianceCalculator(stats: Array[Double]) extends ImpurityCalculator(stats) {
   ...
   */
  def predict: Double = if (stats(3) == 0) {
    0
  } else {
    stats(1) / stats(3)
  }
  ...
  override def toString: String = {
    s"VarianceAggregator(cnt = ${stats(0)}, sum = ${stats(1)}, sum2 = ${stats(2)}, sumWeight = ${stats(3)})"
  }
